...
def main():
    ...
    parser.add_argument('--sizes', default='64,256,1024,4096,16384,65536')
    ...
    if info.get('fftw_available') != 'yes':
        raise SystemExit('FFTW runtime is required for the formal kernel experiment')
    if info.get('avx2_fma') != 'yes' or info.get('avx512_fma') != 'yes':
        raise SystemExit('formal matrix requires AVX2/FMA and AVX-512/FMA')

    fieldnames = ['session', 'size_order', 'phase', 'backend', 'policy', 'N', 'sample',
                  'mode_order', 'iterations', 'ns_per_transform']
    raw_files = []
    total_rows = 0
    ...
    expected = args.sessions * len(sizes) * (7 * args.samples + 7 * args.setup_samples)
    if total_rows != expected:
        raise SystemExit(f'raw row cardinality mismatch: got {total_rows}, expected {expected}')

    metadata = {
        'schema_version': 1,
        'experiment': 'simd-kernel-v1',
        ...
        'avx2_fma': info.get('avx2_fma'),
        'avx512_fma': info.get('avx512_fma'),
        'sizes': sizes,
        ...
        'timing_semantics': 'Persistent forward+inverse pairs; elapsed time divided by two; inverse normalization included; buffers/plans outside execution timing; setup measured separately.',
        'raw_files': [
            {'path': str(path.relative_to(args.out)), 'sha256': sha256(path),
