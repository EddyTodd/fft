...
class Plan {
    using Storage = std::variant<std::monostate, Radix2Plan<T>, MixedRadixPlan<T>, GoodThomasPlan<T>, RaderPlan<T>, BluesteinPlan<T>>;
public:
    explicit Plan(std::size_t n, PlanOptions options = {}) : n_(n), storage_(make_storage(n, options, algorithm_)) {}
    ...
    [[nodiscard]] std::size_t scratch_size() const noexcept {
        return std::visit([](const auto& plan) -> std::size_t {
            using P = std::decay_t<decltype(plan)>;
            if constexpr (std::is_same_v<P, std::monostate> || std::is_same_v<P, Radix2Plan<T>>) return 0;
            else return plan.scratch_size();
        }, storage_);
    }
    [[nodiscard]] std::size_t inplace_scratch_size() const noexcept {
        return std::visit([&](const auto& plan) -> std::size_t {
            using P = std::decay_t<decltype(plan)>;
            if constexpr (std::is_same_v<P, RaderPlan<T>> || std::is_same_v<P, BluesteinPlan<T>>)
                return plan.scratch_size() + n_;
            else if constexpr (std::is_same_v<P, std::monostate> || std::is_same_v<P, Radix2Plan<T>>)
                return 0;
            else return plan.scratch_size();
        }, storage_);
    }
    ...
    void forward(std::span<const ComplexT<T>> input, std::span<ComplexT<T>> output,
                 std::span<ComplexT<T>> scratch = {}) const { execute(input, output, scratch, Direction::Forward); }
    void inverse(std::span<const ComplexT<T>> input, std::span<ComplexT<T>> output,
                 std::span<ComplexT<T>> scratch = {}) const { execute(input, output, scratch, Direction::Inverse); }
...
private:
    static Storage make_storage(std::size_t n, PlanOptions options, PlanAlgorithm& algorithm) {
        if (n <= 1) { algorithm = PlanAlgorithm::Identity; return {}; }
        ...
        if (force == PlanPreference::Radix2) {
            ...
            algorithm = PlanAlgorithm::Radix2; return Storage{std::in_place_type<Radix2Plan<T>>, n};
        }
        if (force == PlanPreference::MixedRadix) {
            ...
            algorithm = PlanAlgorithm::MixedRadix; return Storage{std::in_place_type<MixedRadixPlan<T>>, n};
        }
        if (force == PlanPreference::GoodThomas) {
            ...
            algorithm = PlanAlgorithm::GoodThomas; return Storage{std::in_place_type<GoodThomasPlan<T>>, n};
        }
        if (force == PlanPreference::Rader) {
            if (!caps.rader) throw std::invalid_argument("forced Rader plan requires prime N >= 3");
            algorithm = PlanAlgorithm::Rader; return Storage{std::in_place_type<RaderPlan<T>>, n};
        }
        if (force == PlanPreference::Bluestein) {
            algorithm = PlanAlgorithm::Bluestein; return Storage{std::in_place_type<BluesteinPlan<T>>, n};
        }
        if (caps.radix2) { algorithm = PlanAlgorithm::Radix2; return Storage{std::in_place_type<Radix2Plan<T>>, n}; }
        if (!is_prime(n)) {
        ...
        if (rader_m < blue_m) { algorithm = PlanAlgorithm::Rader; return Storage{std::in_place_type<RaderPlan<T>>, n}; }
        algorithm = PlanAlgorithm::Bluestein; return Storage{std::in_place_type<BluesteinPlan<T>>, n};
    }

    void execute(std::span<const ComplexT<T>> input, std::span<ComplexT<T>> output,
                 std::span<ComplexT<T>> scratch, Direction d) const {
        if (input.size() != n_ || output.size() != n_ || scratch.size() < scratch_size())
            throw std::invalid_argument("Plan buffer size mismatch");
        if (n_ <= 1) { if (n_ == 1) output[0] = input[0]; return; }
        std::visit([&](const auto& plan) {
            using P = std::decay_t<decltype(plan)>;
            if constexpr (std::is_same_v<P, std::monostate>) return;
            else if constexpr (std::is_same_v<P, Radix2Plan<T>>) {
                if (d == Direction::Forward) plan.forward(input, output); else plan.inverse(input, output);
            } else {
                if (d == Direction::Forward) plan.forward(input, output, scratch); else plan.inverse(input, output, scratch);
            }
        }, storage_);
    }

    void execute_inplace(std::span<ComplexT<T>> data, std::span<ComplexT<T>> scratch, Direction d) const {
        if (data.size() != n_ || scratch.size() < inplace_scratch_size()) throw std::invalid_argument("Plan buffer size mismatch");
        if (n_ <= 1) return;
        std::visit([&](const auto& plan) {
            using P = std::decay_t<decltype(plan)>;
            if constexpr (std::is_same_v<P, std::monostate>) return;
            else if constexpr (std::is_same_v<P, Radix2Plan<T>>) {
                if (d == Direction::Forward) plan.forward_inplace(data); else plan.inverse_inplace(data);
                ...
                auto algorithm_scratch = scratch.subspan(n_, plan.scratch_size());
                if (d == Direction::Forward) plan.forward(data, temp, algorithm_scratch);
